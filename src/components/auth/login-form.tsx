...
import { Mail, Lock, Eye, EyeOff, Loader2, AlertTriangle, CheckCircle } from 'lucide-react';
...
type StatusInfo = { status: Status; message: string };
...
const StatusOverlay = ({ status, message }: StatusInfo) => (
    <motion.div
        initial={{ opacity: 0, scale: 0.8 }}
        animate={{ opacity: 1, scale: 1 }}
        exit={{ opacity: 0, scale: 0.8 }}
        className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-background/80 backdrop-blur-sm p-4 rounded-md"
    >
        <div className={`p-4 rounded-full border-4 ${status === 'success' ? 'border-green-500/50' : 'border-destructive/50'}`}>
            {statusIcons[status as 'success' | 'error']}
        </div>
...
export function LoginForm() {
  ...
  const [statusInfo, setStatusInfo] = React.useState<StatusInfo>({ status: 'idle', message: '' });
  ...
  async function onSubmit(data: LoginFormValues) {
    ...
    try {
      ...
      if (firebaseUser) {
        ...
        setStatusInfo({ status: 'success', message: 'Login Successful!' });
        
        setTimeout(() => {
          if (userRole === 'admin') { window.location.assign('/admin/dashboard'); } 
              ...
              if (firstAllowedPage) { window.location.assign(firstAllowedPage.href); } 
              else { toast({ title: "No Permissions", description: "You have not been assigned any delegate permissions.", variant: "destructive" }); window.location.assign('/'); }
      ...
      else if (error.code === 'auth/too-many-requests') errorMessage = "Too many login attempts. Please try again later or reset your password.";
      setStatusInfo({ status: 'error', message: errorMessage });
      setTimeout(() => setStatusInfo({ status: 'idle', message: '' }), 2000);
    }
  }
  ...
  return (
    <div className="relative">
      <AnimatePresence>
        {statusInfo.status !== 'idle' && statusInfo.status !== 'loading' && (
          <StatusOverlay status={statusInfo.status} message={statusInfo.message} />
        )}
      </AnimatePresence>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <FormField
            control={form.control} name="emailOrPhone" render={({ field }) => (
              <FormItem>
                ...
                <div className="relative group">
                  ...
                  <FormControl><Input type="email" placeholder="your@email.com" {...field} className="pl-10 py-3 text-base bg-background/70 border-border/50 focus:border-accent focus:ring-accent/50 shadow-inner"/></FormControl>
                </div>
                <FormMessage />
              </FormItem>
            )}
              ...
              <FormItem>
                <FormLabel className="text-muted-foreground">Password</FormLabel>
                 <div className="relative group">
                  ...
                  <FormControl><Input type={showPassword ? "text" : "password"} placeholder="••••••••" {...field} className="pl-10 pr-10 py-3 text-base bg-background/70 border-border/50 focus:border-accent focus:ring-accent/50 shadow-inner"/></FormControl>
          ...
          <div className="flex items-center justify-between text-sm">
            <FormField
              control={form.control} name="rememberMe" render={({ field }) => (
                <FormItem className="flex flex-row items-center space-x-2.5 space-y-0">
                  <FormControl><Checkbox checked={field.value} onCheckedChange={field.onChange} className="border-primary data-[state=checked]:bg-accent data-[state=checked]:border-accent h-5 w-5" id="rememberMeLogin"/></FormControl>
                  <Label htmlFor="rememberMeLogin" className="font-normal text-muted-foreground cursor-pointer">Remember me</Label>
                </FormItem>
              )}
            />
            <Link href="/auth/forgot-password" legacyBehavior><a className="text-accent hover:underline hover:text-accent/80 transition-colors">Forgot password?</a></Link>
          </div>
