...
import TournamentUserList from './tournament-user-list';
...
import {
  ...
  UserPlus, Loader2, AlertTriangle, MessagesSquare, CheckCircle2, KeyRound, Eye, EyeOff, Copy, Zap
} from 'lucide-react'; 
...
const TournamentDetailPageClient: React.FC<TournamentDetailPageClientProps> = ({ initialTournament }) => {
  ...
  const handleAutoPostResults = useCallback(async () => {
    if (!database || !id || currentTournament.status === 'completed' || currentTournament.autoPostCompleted) return;
    console.log(`Auto-posting results for tournament ${id}`);
    const updates: Partial<Tournament> = { status: 'completed', resultsPosted: true, autoPostCompleted: true };
    try {
        await update(ref(database, `tournaments/${id}`), updates);
        toast({ title: "Results Posted", description: `Results for "${name}" are now live.` });
    } catch (error) {
        console.error("Failed to auto-post results:", error);
    }
  }, [id, currentTournament, name, toast]);
  ...
  const { id, name, game, mode, entryFee, prizePool, perKillReward, maxPlayers, joinedPlayersCount = 0, status, startTime, youtubeLive, customRules, bannerImageUrl, playersJoined = {}, roomId, roomPassword, resultsProcessingTime, autoPostResults } = currentTournament || initialTournament;
  ...
  useEffect(() => {
    if (parsedStartTime) setIsCountdownFinished(!isFuture(parsedStartTime));
  }, [startTime]);
        ...
        <div className="md:col-span-2 space-y-6">
          <GlassCard>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4">
              ...
              {showCountdown && <CountdownTimer targetDate={startTime} className="text-accent" size="lg" onComplete={() => setIsCountdownFinished(true)} />}
            </div>
            <Separator className="my-4 bg-border/50" />
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm"><InfoItem icon={Shield} label="Mode" value={mode} /><InfoItem icon={RupeeIcon} label="Entry Fee" value={entryFee > 0 ? `Rs ${entryFee}` : 'Free'} /><InfoItem icon={Award} label="Prize Pool" value={`Rs ${prizePool}`} /><InfoItem icon={Zap} label="Per Kill Reward" value={`Rs ${perKillReward}`} /><InfoItem icon={Users} label="Players" value={`${joinedPlayersCount || 0} / ${maxPlayers}`} /><InfoItem icon={CalendarDays} label="Start Time" value={parsedStartTime ? new Date(startTime).toLocaleString() : 'Date TBD'} /></div>
          </GlassCard>
          ...
          {showAutoPostCountdown && <GlassCard><h2 className="text-xl font-semibold text-foreground mb-2 flex items-center"><Clock className="mr-2 h-5 w-5 text-accent"/> Time Until Results</h2><CountdownTimer targetDate={resultsProcessingTime!} onComplete={handleAutoPostResults} className="text-accent" size="lg"/></GlassCard>}
