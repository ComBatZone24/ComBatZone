      ...
      <GlassCard>
        <FormProvider {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6"><FormField name="name" label="Tournament Name" form={form} /><FormField name="game" label="Game Title" form={form} /></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6"><FormField name="map" label="Map (Optional)" form={form} /><Controller name="mode" control={form.control} render={({ field }) => ( <FormItem field={field} label="Game Mode"><Select onValueChange={field.onChange} value={field.value || ""}><FormControl><SelectTrigger className="bg-input/50"><SelectValue placeholder="Select mode" /></SelectTrigger></FormControl><SelectContent className="glass-card"><SelectItem value="Solo">Solo</SelectItem><SelectItem value="Duo">Duo</SelectItem><SelectItem value="Squad">Squad</SelectItem><SelectItem value="Custom">Custom</SelectItem></SelectContent></Select></FormItem> )}/></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6"><FormField name="maxPlayers" type="number" label="Max Players" form={form} /><Controller name="status" control={form.control} render={({ field }) => ( <FormItem field={field} label="Tournament Status"><Select onValueChange={field.onChange} value={field.value || "upcoming"}><FormControl><SelectTrigger className="bg-input/50"><SelectValue placeholder="Select status" /></SelectTrigger></FormControl><SelectContent className="glass-card"><SelectItem value="upcoming">Upcoming</SelectItem><SelectItem value="live">Live</SelectItem><SelectItem value="completed">Completed</SelectItem><SelectItem value="archived">Archived</SelectItem></SelectContent></Select></FormItem> )}/></div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6"><FormField name="entryFee" type="number" label="Entry Fee" form={form} /><FormField name="prizePool" type="number" label="Prize Pool" form={form} /><FormField name="perKillReward" type="number" label="Per Kill Reward" form={form} /></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6"><FormField name="startDate" type="date" label="Start Date" form={form} /><FormField name="startTime" type="time" label="Start Time" form={form} /></div>
            ...
            <Separator className="my-4 bg-border/30" /><h3 className="text-lg font-semibold text-foreground -mb-2 flex items-center"><Clock className="mr-2 h-5 w-5 text-accent"/>Auto-Post Results Settings</h3>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-end"><FormField name="resultsProcessingTimeDate" type="date" label="Auto-Post Date" form={form} /><FormField name="resultsProcessingTimeTime" type="time" label="Auto-Post Time" form={form} /></div>
            ...
            <Separator className="my-4 bg-border/30" /><FormField name="bannerImageUrl" label="Banner Image URL (Optional)" form={form} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6"><FormField name="roomId" label="Room ID" form={form} /><FormField name="roomPassword" label="Room Password" form={form} /></div>
...
const FormField: React.FC<{name: keyof TournamentFormValues; label: string; placeholder?: string; type?: string; form: ReturnType<typeof useForm<TournamentFormValues>>}> = ({ name, label, placeholder, type = "text", form: currentForm }) => {
