...
import { ref, set, onValue, off, remove, get } from 'firebase/database'; // Added onValue, off, remove and get

// Helper component, can be moved to a shared location if used by many settings pages
...
export default function AdminRedeemCodePage() {
  ...
  const handleGenerateRedeemCode = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const newCodeEntry: RedeemCodeEntry = {
        ...
        isUsed: false, // This will be true if timesUsed >= maxUses after a claim
        usedBy: null, // Deprecated if claimedBy is used extensively, or for first user on maxUses=1
        createdAt: new Date().toISOString(),
        maxUses: numericMaxUses,
        timesUsed: 0,
        claimedBy: {}, // Initialize as empty object for tracking multiple claims
      };
  ...
  const getCodeStatus = (code: RedeemCodeEntry & { codeString: string }) => {
    ...
    // Add expiry check here later if implemented
  ...
  return (
    <AlertDialog open={isDeleteDialogOpen} onOpenChange={(isOpen) => {
      ...
      if (!isOpen) setCodeToDelete(null);
    }}>
      <div className="space-y-8">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-3xl md:text-4xl font-bold text-foreground flex items-center">
              <Ticket className="mr-3 h-8 w-8 text-accent"/> Redeem Code Management
          </h1>
        ...
        <GlassCard>
          ...
          <p className="text-sm text-muted-foreground mb-4">
            Generate codes for users. The redeem system must be enabled in "General Features".
          </p>
        ...
        <AlertDialogContent className="glass-card">
          {codeToDelete && (
            <>
              <AlertDialogHeader>
                <AlertDialogTitle>Delete Redeem Code: {codeToDelete.codeString}?</AlertDialogTitle>
                <AlertDialogDescription>
                  This action cannot be undone. This will permanently delete the redeem code
                  <span className="font-semibold text-foreground"> "{codeToDelete.codeString}"</span>.
                  Users will no longer be able to claim it.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
